...
def generate(input_file, output_folder, zoom_level, resize_width, quiet):
    ...
    try:
        if input_file.endswith('.jpg') or input_file.endswith('.jpeg'):
            ...
            if not quiet:
                print(
                    'ERROR: Unknown source image type; JPG, TIFF, or PNG only! Quitting...')
            exit(0)
    except:
        if not quiet:
    ...
    if not power_of(int(width), 2):
        if not quiet:
            print('WARNING: Source image dims should be power of 2! Continuing anyway...')
    if width != height:
        if not quiet:
            print('ERROR: Source image should be square! Quitting...')
        exit(0)
    ...
    if not quiet:
        print('- ' + str(tile_width) + ' x ' + str(tile_width) + ' px tiles')
    ...
    cmd = 'convert ' + input_file + ' -quiet -crop ' + str(tile_width) + 'x' + str(tile_width) + ' -resize ' + str(
        resize_width) + 'x' + str(resize_width) + ' ' + output_folder + '/%0' + str(pad) + 'd.png'
    os.popen(cmd)
    if not quiet:
        print('- done!')
    ...
    if not quiet:
        print('Moving files into column folders...')
    ...
    for i, tile in enumerate(tiles):
        ...
        f = i / num_tiles
        dst = output_folder + '/' + \
            str(zoom_level) + '/' + str(col) + '/' + str(f) + '.png'
        print(tile)
        print(dst)
...
if __name__ == '__main__':
    p = argparse.ArgumentParser(description='Takes a large image as the input, outputs map tiles at the appropriate size and file structure for use in frameworks like leaflet.js, MapBox, etc. Much more info in the source code.',
                                usage='python GenerateSlippyMapTiles.py input_file zoom_level output_folder [options]')
    p.add_argument(
        'input_file', help='large image file to split (JPG, PNG, or TIFF)')
    p.add_argument(
        'zoom_level', help='zoom level(s) to generate (0 to 18); either integer or range (ex: 2-6)')
    p.add_argument(
        'output_folder', help='folder name to write tiles to (will be created if does not exist)')
    p.add_argument('-w', '--resize_width', help='dimension in pixels for outputted tiles (default 256px)',
                   metavar='', type=int, default=256)
    p.add_argument(
        '-q', '--quiet', help='suppress all output from program (useful for integrating into larger projects)', action='store_true')
    args = p.parse_args()
    ...
    if not quiet:
        print('GENERATING SLIPPY-MAP TILES')
    if not quiet:
        print('- ' * 14)
    ...
    if '-' in zoom_level:
        try:
            ...
            if not quiet:
                print(
                    "ERROR: Couldn't parse zoom levels; should be int or 'min-max'! Quitting...")
            exit(0)
        ...
        generate(input_file, output_folder, int(
            zoom_level), resize_width, quiet)
        if not quiet:
            print('- ' * 14)
